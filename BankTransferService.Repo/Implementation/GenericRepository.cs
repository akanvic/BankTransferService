...
namespace BankTransferService.Repo.Implementation
{
    public class GenericRepository<TEntity> : IDPGenericRepository<TEntity> where TEntity : class
    {
        ...
        public async Task<TEntity> QueryFirstOrDefaultAsyncWithTypeSp<T>(string sql, CommandType commandType, object parameters = null)
        {
            /*_logger.LogDebug($"QUERY GET COMMAND | { sql.Trim() }");
            _logger.LogDebug($"QUERY GET PARAMETERS | { parameters }");*/
        ...
        public async Task<T> QueryFirstOrDefaultAsyncWithTypeSpV2<T>(string sql, CommandType commandType, object parameters = null)
        {
            /*_logger.LogDebug($"QUERY GET COMMAND | { sql.Trim() }");
            _logger.LogDebug($"QUERY GET PARAMETERS | { parameters }");*/
